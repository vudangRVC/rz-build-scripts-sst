...
source include/01_prepare_debian_base.sh
...
function main(){
    ...
    # Prepare the Debian base system (install necessary binaries and setup rootfs)
    debian_base_prepare
    if [[ $? -eq 1 ]]; then
        echo "debian_base_prepare failed."
        exit 1
    fi

    # Prepare qt_rootfs_source by copying relevant binaries and folders to the Debian OS
    rootfs_qt
    if [[ $? -eq 1 ]]; then
