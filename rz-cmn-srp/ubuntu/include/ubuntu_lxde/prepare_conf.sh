...
copy_qt() {
	...
	cp -rd "$src_qt/lib/libicui18n.so.75"* "$src_qt/lib/libicuuc.so.75"* "$src_qt/lib/libicudata.so.75"* "$target_dir/lib/aarch64-linux-gnu/" || { echo "Failed to copy to directory"; return 1; }
...
set_config_after_install() {
	...
	echo "set_dns_config completed successfully."
