...
log_info(){
	string=$1
	echo -e "\033[32;1;24m${string}\033[0m"
}
		...
		for patch_path in $patch_list; do
			if [ -n "$patch_path" ]; then
				...
				if [ ! -e "${patch_path}" ]; then
					echo "Error: Patch ${patch_path} is not present in this workspace (${patch_path})."
...
bsp_checkout_verification() {
	...
	cd "${RZ_TARGET_DIR}" || { echo "Failed to change directory to ${RZ_TARGET_DIR}"; exit 1;}
	...
	for bsp_layer in $bsp_layers; do
		...
		cd .. || { log_error "Failed to return to parent directory"; exit 1; }
	done
...
check_and_clone_missing_layers() {
	...
	for missing_layer in $missing_layers; do
		...
		if [ "$repo_type" = "local" ]; then
			...
			cd "${missing_layer}" || { log_error "Failed to change dir ${missing_layer}" ; exit; }
...
extract_to_meta(){
	...
	cd "${WORKSPACE}" || { log_error "Failed to switch to ${WORKSPACE}" ; exit 1; }
	pwd
	unzip "${zipfile}"
...
build_sdk() {
	...
	case "${IMAGE}" in
		"all-supported-images")
			# If IMAGE is set to 'all-supported-images', build SDK for all images
		...
		;;
		"all-yocto-images")
			# If IMAGE is set to 'all-yocto-images', build SDK for all images
			log_info "Building SDK for all supported 'yocto' images..."
		...
		;;
		*)
			# If IMAGE is set to individual images, build SDK for selected images
			log_info "Building SDK for selected image : '${IMAGE}'..."
			...
			${JQ} -r --arg value "${IMAGE}" 'to_entries | map(select(.value | index($value) != null)) | .[].key' "${IMAGES_JSON}" | while IFS= read -r entry; do
				if [ "${entry}" = "yocto" ]; then
					...
					echo "Unidentified entry : ${entry}. skipping."
				fi
			done
...
build() {
	...
	case "${IMAGE}" in
		"all-supported-images")
			...
			# If IMAGE is set to 'all-yocto-images', build all images
			log_info "Building all supported 'yocto' images..."
		...
		;;
		"all-ubuntu-images")
			# If IMAGE is set to 'all-ubuntu-images', build all images
			log_info "Building all supported 'ubuntu' images..."
			...
			log_info "Finished building all supported ubuntu based images."
			log_info "========================================================================"
		;;
		"all-yocto-images")
			# If IMAGE is set to 'all-yocto-images', build all images
		...
		;;
		*)
			# If IMAGE is set to individual images, build all images
			log_info "Building selected image : '${IMAGE}'..."
...
if ! command -v ${JQ} >/dev/null 2>&1; then
	echo "Error: ${JQ} shell utility is missing. Please install jq to use this script."
	exit 1
fi
...
image_found=0
#option_error=0
# Iterate over all values in the JSON file
while IFS= read -r value; do
  if [ "${value}" = "${IMAGE}" ]; then
	echo "Image: '${IMAGE}' found in the JSON file."
	image_found=1
	#if build type and sdk is not provided.
	if [ -z "${1}" ] ; then
		guideline
	else
		if [ "${1}" = "build" ]; then
			...
			echo "Invalid parameter for build type."
			guideline
#			((option_error++))
		fi
	fi
...
done < /tmp/build-images.lst

if [ ${image_found} = 0 ]; then
	echo "Value ${IMAGE} not found in the JSON file."
	guideline
fi
