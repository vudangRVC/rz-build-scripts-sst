...
bsp_checkout_verification() {
	...
	for bsp_layer in $bsp_layers; do
		...
		# Only verify the git repositories that have enabled in JSON file
...
check_and_clone_missing_layers() {
	...
	for missing_layer in $missing_layers; do
		...
		# Only clone the git repositories enabled in git_patch.json file
		if [ "$is_layer_enabled" = "false" ]; then
			continue
		fi
...
build() {
	...
	if [ -z "$DISTRO" ]; then
		...
		MACHINE=rzg2l-sbc DISTRO="${DISTRO}" bitbake "${IMAGE}"
	fi
